...
> {
    ...
    constructor(client: Client, agentId: string) {
        this.client = client;
    ...
    static async create<
        TInput = Record<string, unknown>,
        TOutput = Record<string, unknown>,
    >(client: Client, config: AgentConfig): Promise<Agent<TInput, TOutput>> {
    ...
    static async load<
        TInput = Record<string, unknown>,
        TOutput = Record<string, unknown>,
    >(client: Client, agentId: string): Promise<Agent<TInput, TOutput>> {
    ...
    async run(
        ...
        // Create the run
        const run = await this.client.createAgentRun(
            this.agentId,
            input as CreateAgentRunRequest
        );

        // Wait for completion
        const completedRun = await this.client.waitForAgentRun(
            this.agentId,
            run.id,
            options
        );
    ...
    async runBatch(
        ...
        const runIds = await Promise.all(
            inputs.map((input) => this.startRun(input))
        );

        // Wait for all runs to complete
        const results = await Promise.all(
            runIds.map((runId) => this.waitForRun(runId, options))
        );
